...
def main():
    ...
    for label in LABEL_NAMES:
        ...
        collapsed, reason = _detect_collapse(
            calib_stats, args.min_calibrated_unique, args.max_zero_fraction
        )
        use_blend = False
        if collapsed:
            use_blend = True
            y_calib = args.blend_weight * y_pred + (1.0 - args.blend_weight) * y_calib
            calib_stats = _value_stats(y_calib)
            print(f"[calibration] Blend fallback applied for {stage}: {reason}")
        ...
        calibration_report["milestones"][stage] = {
            ...
            **metrics,
            ...
            "blend_applied": use_blend,
            "blend_weight": args.blend_weight if use_blend else 0.0,
        }
        ...
        metadata["milestones"][stage] = {
            ...
            "positives": positives,
            "blend_applied": use_blend,
            "blend_weight": args.blend_weight if use_blend else None,
        }
